...
def customer_mart_calculation_table_write(data_df):
    window = Window.partitionBy("customer_id","sales_date_month")
    data_df = data_df.withColumn("sales_date_month",
    ...
    db_writer.write_dataframe(data_df,config.customer_data_mart_table)
